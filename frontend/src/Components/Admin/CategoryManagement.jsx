...
const CategoryManagement = () => {
  ...
  const handleUpdateCategory = async (e) => {
    ...
    try {
      await ticketService.updateCategory(categories[editingCategory.index]._id, editingCategory.name);
      const updatedCategories = await ticketService.getCategories();
      setCategories(updatedCategories);
      setEditingCategory({ index: -1, name: '' });
    } catch (err) {
                ...
                {editingCategory.index === index ? (
                  ...
                  <div className="category-display">
                    ...
                    <div className="category-actions">
                      <button onClick={() => handleEditClick(index, category.name)} className="btn-secondary">
                        Edit
                      </button>
                      <button onClick={() => handleDeleteCategory(category._id)} className="btn-danger">
                        Delete
                      </button>
                    </div>
                  </div>
